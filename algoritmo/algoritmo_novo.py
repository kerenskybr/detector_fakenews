...
verdadeira['corpo'] = verdadeira['corpo'].map(lambda x: str(x)[:300])

falsa['corpo'] = falsa['corpo'].map(lambda y: str(y)[:300])
...
tfidf = TfidfVectorizer(min_df = 1, strip_accents = 'unicode', max_features = 3000,
                        analyzer = 'word', ngram_range = (1,3), sublinear_tf = 1, encoding='utf-8', stop_words = pt_stopwords)
...
#Acerto miseravi # texto = ['Uma senhora foi agredida no Rio de Janeiro ao gritar Bolsonaro. A foto da senhora com o rosto machucado choca o eleitor. Mas o que choca mesmo é que a senhora é Beatriz Segall, morta no início de setembro.']

#texto = texto.str.replace('""=(){$%[^\w\s]','')

#texto = texto.str.lower()

carrega_modelo = joblib.load('naive_test.sav')
